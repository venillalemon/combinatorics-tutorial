\chapter{Combinatorics 3.24}
\label{ch:250324}

\section{flipping a coin}

\notefigure{notes/250324-01.png}
...
\begin{quotation}
\textbf{Thm.} (Turan, 1941) n 阶图 $\omega\leq p$, 那么 $m\leq t(n,p)$, $t(n,p)$ 为完全 $p$ 部图 $T(n,p)$ 的边数.
\end{quotation}

\notefigure{notes/250324-02.png}

我们可估计 $t(n,p)\sim (1-\frac{1}{p})\frac{n^2}{2}$, 一种方法是直接计算，还有一种是看每一个点都被砍了 $1/p$ 条出边.

\textbf{证1.} (from the last proof of Mantel's Thm)
...
\textbf{证2.} Induction. 若 $n\leq p$ 自然成立 ($K_n$); $n>p$ 假设 $G$ 是 $m$ 最大的 $n$ 阶 $\omega\leq p$ 图. 有 $\omega(G)=p$ (否则，若 $\omega(G)<p$, 随便加一条边，$\omega$ 至多增加1). 挑出这个 $K_p$ 和其他点，$m\leq{p\choose2}+t(n-p,p)+(n-p)(p-1)$.

\notefigure{notes/250324-03.png}
...
\textbf{证3.} 设我们要的图是 $G$, 即 $G$ 满足 n 阶，$\omega\leq p$ 且 m 最大. 往证 $G$ 是完全某部图.
...
否则可以通过复制的方法增加边数. 若 $\deg x<\deg y$, 考虑 $G-x+y^\prime$, 首先复制一个 $y$ 不会增加 $\omega$.

填空题，你可以填 “矛盾” 🤣🤣

\notefigure{notes/250324-04.png}

call back 我们关于 induced $P_3$ 的讨论. 如果不是完全某部图，那么其补图不是每个连通块都是团，因此存在 induced $P_3$. 因此应该填 \textbf{原图存在 induced · |} , 所以根据 Lemma 有 $\deg u=\deg v=\deg w$.

\notefigure{notes/250324-05.png}

\textbf{证4.} (Erdos)

v 为度数最大的点，$A=N(v),B=V\setminus N(v)$，$A$ 中没有 p-团. 先由归纳假设，将 $A$ 补成 $T(|A|,p-1)$, $B$ 国内边全去掉, 然后将 $A,B$ 的点全部连起来形成 $H$.

\notefigure{notes/250324-06.png}
...
\begin{quotation}
\textbf{Lemma.} 对任意图 $G$, $\sum_{v\in G}\frac{1}{\deg v+1}\leq\alpha$.
\end{quotation}
