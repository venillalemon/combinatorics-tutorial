...
\begin{quotation}
\textbf{Thm.} (1980, Crossing Lemma) $\text{Cr}(G)\geq\frac{m^3}{64n^2}(m\geq 4n)$
\end{quotation}
...
\section{crossing lemma 应用: 好边}

\begin{quotation}
\textbf{Thm.} (Szemeredi-Trotter) $n$ points $\in\mathbb{R}^2$, 定义一条直线为好边当且仅当线上有 $\geq k\;(2\leq k\leq \sqrt{n})$ 个点. 这样的好边最多 $O\big(\frac{n^2}{k^3}\big)$ 个.
\end{quotation}

将好线上的相邻点 (consecutive) 连边，点数 $n$, 边数 $\geq l(k-1)$, 交叉数 $\leq{l\choose 2}$. 比较 $m$ 和 $4n$ 大小.

已知 $k\leq \sqrt{n}$, $m<4n\Rightarrow l<\frac{4n}{k-1}\leq O\big(\frac{n^2}{k^3}\big)$
...
\section{cl 应用：number of incidences}

$I(P,L)=\{(p,l):p\in P,l\in L,p\in l\}$

\begin{quotation}
\textbf{Thm.} (Szemeredi-Trotter) $|P|=n,|L|=m,|I(P,L)|\leq 4(m^{\frac{2}{3}}n^{\frac{2}{3}}+m+n)$
\end{quotation}

连接线上的相邻点，不放假设每条边至少1个点（否则直接删除，$m$ 减少，不影响待证不等式）。点数 $n$, 边数：每条线如果有 $n>1$ 个点，那么有 $n-1$ 条边，共 $|I|-m$ 条. 交点最多 ${m\choose 2}$

crossing lemma: 要么 $|I|-m<4n$, 要么 $\frac{m^2}{2}\geq\text{Cr}\geq\frac{(|I|-m)^3}{64n^2}$

\section{cl 应用：decent graph 最大边数}

erdos：不存在 $K_{2,3}$，因此 $m<n^{1.5}$
...
\section{cl 应用：distinct distances}

平面上 $n$ 个点，至少距离种类数

\notefigure{notes/250519-01.png}
...
对任意的图 $G$, 第一阶段，每个边以 1/k 概率选取；第二阶段，每个重复的只选一个. 每条边留下来的概率<=1/k

Z是这个画法的crossing $\mathbb{E}[Z]\leq Cr/k^2$

每条边活过两个阶段的概率<1/k. Y为边数，每一条边，如果第一阶段唯一的活下来，就成功，因此有留下概率下界 $\frac{m}{k}(1-\frac{1}{k})^{k-1}\leq E[Y]\leq \frac{m}{k}$
